grep -R -H -E "esxcli system account" . \
  | grep -E "\-i |--id" \
  | grep -Ev "shell" \
...
grep -R -H -E "network firewall set" . \
  | grep -E "enabled f" \
  | awk '{ print $0 "   <-- ESXi Firewall Disabled" }'

grep -R -H -E "lockdownmode\.disabled|Administrator access to the host has been enabled" . \
  | awk '{ print $0 "   <-- ESXi Lockdown Mode Disabled" }'

grep -R -H -E "Set called with key" . \
  | grep -E "Syslog\.global\.logHost|Syslog\.global\.logdir" \
  | awk '{ print $0 "   <-- ESXi Loghost Config Tampering" }'

grep -R -H -E "image profile with validation disabled\.|image profile bypassing signing and acceptance level verification\.|vib without valid signature," . \
  | awk '{ print $0 "   <-- ESXi Malicious VIB Forced Install" }'

grep -R -H -E "bash -i >&|/dev/tcp/|/dev/udp/|socat exec:|socket\(S,PF_INET" . \
...
grep -R -H -E "esxcli system permission set" . \
  | grep -E "role Admin" \
